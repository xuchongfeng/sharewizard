...
def work():
    ...
    s_180_day_ago = date_util.get_n_days_ago(180)
    n_day_high = NDayHigh()
    process = 0
    ...
    for company in all_company:
        ...
        if process % 100 == 0:
            print(process)
        price_list = Trade.objects.order_by('-t_date').filter(company=company, t_date__gt=s_180_day_ago.strftime("%Y-%m-%d"),
                                                                 t_date__lt=today.strftime("%Y-%m-%d")).values_list('close')
        ...
        if n_day_high.is_15_day_high(price_list):
            strategy = Strategy(t_date=today, company=company, t_type=Strategy.T_15_DAYS)
            strategy.save()

            s_15_day_high_company.append(company)

        if n_day_high.is_30_day_high(price_list):
            strategy = Strategy(t_date=today, company=company, t_type=Strategy.T_30_DAYS)
            strategy.save()

            s_30_day_high_company.append(company)

        if n_day_high.is_60_day_high(price_list):
            strategy = Strategy(t_date=today, company=company, t_type=Strategy.T_60_DAYS)
            strategy.save()

            s_60_day_high_company.append(company)

        if n_day_high.is_120_day_high(price_list):
            strategy = Strategy(t_date=today, company=company, t_type=Strategy.T_120_DAYS)
            strategy.save()

            s_120_day_high_company.append(company)

    industry_count(s_15_day_high_company, Industry.T_15_DAYS)
    industry_count(s_30_day_high_company, Industry.T_30_DAYS)
    industry_count(s_60_day_high_company, Industry.T_60_DAYS)
    industry_count(s_120_day_high_company, Industry.T_120_DAYS)


def industry_count(company_list, strategy_type):
    today = date_util.today()
    industry_high_count = {}
    ...
    for industry, count in industry_high_count.items():
        ...
        industry_model = Industry(t_date=today, name=industry, t_type=strategy_type, number=count)
        industry_model.save()
